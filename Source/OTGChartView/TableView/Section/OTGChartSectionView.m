...
@interface OTGChartSectionView()<OTGChartDrawViewDelegate>


@end

@implementation OTGChartSectionView

- (void)prepareGesture
{
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tappedSectionView:)];
    [self addGestureRecognizer:tapGesture];
}


- (void)tappedSectionView:(UITapGestureRecognizer *)gesture
{
    if ([self.delegate respondsToSelector:@selector(ganttChartSectionView:tappedSection:)]) {
        [self.delegate ganttChartSectionView:self tappedSection:self.section];
    }
}
...
{
    OTGChartDrawView *drawView = [[OTGChartDrawView alloc]initWithFrame:self.bounds];
...
- (void)locationPointViews
{
    ...
    [self.pointViewArray enumerateObjectsUsingBlock:^(OTGChartPointView *_Nonnull chartPointView, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [weakSelf addSubview:chartPointView];
    }];
}
...
- (OTGChartPointView *)chartPointViewVariableSetting:(OTGChartPointView *)chartPointView
{
    ...
    chartPointView.indexPath = [NSIndexPath indexPathForRow:NSNotFound inSection:self.section];
